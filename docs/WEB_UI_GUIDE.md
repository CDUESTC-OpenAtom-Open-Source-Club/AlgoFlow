...
### 5.3 工作区首页与 HTML 原型对齐记录（Issue #14）

首页 `pages/Home.ets` 以 `idea-flow_home.html`（画布 `393 x 852`）为基准做视觉与交互对齐，遵循「结构复刻、Token 沿用项目规范」原则。

**颜色 Token 映射**（原型 → OH 端）

| 原型 | OH 端 | 用途 |
| --- | --- | --- |
| `#4d8478`（teal 主色） | `#55745F`（项目 primary） | 主操作、选中态、同步成功 |
| `bg-slate-950`（近黑） | `#25352C`（深色表面） | 品牌方块、创建草稿卡 |
| `bg-[#f7f8fa]`（页面底） | `#F7F8F6` | 首页背景 |
| `bg-[#e7f2ef]` / `amber-50` / `slate-200/70` | `#E5F1EB` / `#FAF3DC` / `#E9EEF4` | 三张状态卡浅底 |
| `text-slate-500/400` | `#68736C` / `#8A938D` | 说明文字、元信息 |

> 不把原型的 `#4d8478` 直接写入首页：`#55745F` 是 `docs/WEB_UI_GUIDE.md` 第 2 节已确认的项目主色，Index IDE 与其余页面均沿用，改色会造成跨页不一致。原型的 teal 仅作为视觉参考，映射为项目主色。

**白色图标染色**：`af_*.svg` 均为 Game Icon Pack 的白色单色资源，原型中「浅底 + 彩色图标」无法直接用白图标呈现。`ResourceIcon` 新增可选 `tint` 参数，通过 ArkUI `Image.fillColor` 在渲染期给同一套白图标染色，避免新增/伪造图标资产。深色块（品牌/创建卡）仍用原始白图标。

**日期策略**：原型 `9.3/周三` 为静态演示值。OH 端由 `Home.todayLabel()` 用 `new Date()` 生成真实本地日期，格式 `M.D/周X`，不做硬编码。

**对齐点清单**

- 顶部：深色品牌方块（圆角 12）+ `思流`/`IDEA FLOW / WORKSPACE`；通知入口为白底描边圆钮（原型 `bg-white border`），图标用现有 `query` 资源占位。
- 标题：`把灵感变成` 36 / `可复用的思路` 22（绿色强调），下方真实日期斜体。
- 状态卡：等宽三卡、圆角 20、`icon + 数值 + 标签`，数值来自真实 `Draft` 统计；历史版本保留「待接入」。
- 创建卡：深色主操作卡，左图标 + 标题/副说明 + 右侧箭头语义图标（现有 `source` 资源占位）。
- 最近草稿卡：状态圆点/标签/版本号/标题（`maxLines(1)` + 省略号截断）/摘要/编辑入口/同步时间/归档/删除；圆角 16。
- 底部导航：四项、选中项绿色图标+加粗文字，无背景块；高度 74。

**仍待预研 / 未验证项**

- 通知「铃铛」、创建卡「箭头」、导航「首页/魔棒/设置」等原型图标在已核验 Game Icon Pack 中暂无对应资源，当前用语义最接近的现有图标占位；待补充图标资产与许可证核验后再替换。
- 标题原型使用 `SourceHanSerif` 900 字重，OH 端未新增字体（避免许可证未核验引入资源），用默认字体 `Bold` 近似。
- 历史版本数量仍为「待接入」，未接入真实版本记录。
- 真机触摸、字体放大、小屏安全区、软键盘弹出未在设备验证；Previewer 通过不等于设备验证。
