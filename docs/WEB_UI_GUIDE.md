...
- 本次 ArkUI 对齐仅覆盖 IDE 工作区，不整合尚未完成设计的首页/主页面卡片。
- 参考 HTML 的顶部导航、深色代码区、底部浮动工具胶囊和左侧抽屉是布局参考；HTML/Tailwind 实现不直接移植到 ArkUI。
- 抽屉打开时使用灰黑遮罩；抽屉选中项使用 `#5A5A5F` 灰色边框和 `#ECECED` 高亮字体，绿色只用于同步成功、选中模式和主要操作。
- 主代码区必须继续使用原生 `TextArea`，保留复制、粘贴、撤销/重做、输入法、选区、光标跳转和长文本滚动能力。
- 抽屉、工具胶囊和面板只是 UI 入口变化，不得改变 `DraftWorkspaceViewModel`、RDB 离线副本、操作队列或 `PhoneSyncService` 的调用语义。
- 主页面后续通过独立页面/入口接入；IDE 页面先保留可替换的页面编排边界，不在本次 UI 对齐中混入首页内容。
